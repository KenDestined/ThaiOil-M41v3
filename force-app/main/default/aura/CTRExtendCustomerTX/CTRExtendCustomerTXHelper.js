({
    ...
    saveRequest: function(component) {
        console.log('Form item',this.getFormData(component))
    ...
    prepopulateHeader: function(component) {
        ...
        if(component.get('v.isEdit')) {
            ...
            console.log('prepopulate header accObj ',accountObject)
            if(!$A.util.isEmpty(accountObject)) {
                ...
                component.find("editFormHeaderField").forEach(function(cmp) {
                    var fieldName = cmp.get('v.fieldName');
                    var value;
                    console.log('Debug popHeader',fieldName,value)
                    if(["CustomersFacilityTankAndStorage__c", "CustomersFacilityTransportation__c", "SuppliersFacilityTankAndStorage__c", "SuppliersFacilityTransportation__c","SupplierSupplyProductTo__c","CustomerPurchaseProductFrom__c"].includes(fieldName)) {
                        value = accountObject[fieldName];
                    }
                    // console.log('[prepopulateHeader] ' + fieldName + ' -----', value);
                    if(!$A.util.isEmpty(value)) {
    ...
    prepopulateItem: function(component) {
        ...
        if(!$A.util.isEmpty(accountObject)) {
            component.find("editFormItemField").forEach(function(cmp) {
                ...
                // {
                //     cmp.set('v.value','a3t1m000002FQF3AAO');
