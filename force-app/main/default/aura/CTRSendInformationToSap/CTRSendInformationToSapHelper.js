({
    ...
    handleState: function (component, helper, result) {
        ...
        var EffectiveDate = mItem.EffectiveDate__c;
        var EffectiveType = mItem.EffectiveType__c;
        var today = new Date();
        var dd = String(today.getDate()).padStart(2, '0');
        var mm = String(today.getMonth() + 1).padStart(2, '0'); //January is 0!
        var yyyy = today.getFullYear();
        today = yyyy + '-' + mm + '-' + dd;
        var isEffectiveDateFuture = mItem.EffectiveDate__c ? mItem.EffectiveDate__c > today : false;
        console.log('####Debug effective date ',mItem.EffectiveDate__c,today,isEffectiveDateFuture)
        ...
        console.log('SAPStatus:', SAPStatus, 'SAPNumber:', SAPNumber, 'SAPCCAMapped:', SAPCCAMapped, 'recordType:', recordType, 'isGeneralChanged:', isGeneralChanged, 'inQueueCCA:',inQueueCCA, 'AccountNumber:',AccountNumber, 'EffectiveType:',EffectiveType,'EffectiveDate__c:',EffectiveDate)
        ...
        for (var i = 0; i < configState.length; i++) {
            ...
            for (var j = 0; j < state.conditions.length; j++) {
                ...
                if ('isEffectiveDateFuture' in conditions && conditions.isEffectiveDateFuture != isEffectiveDateFuture) {
                    continue;
                }
                ...
                if ('EffectiveType__c' in conditions) {
                    if (conditions.EffectiveType__c == 'not null') {
                        if(!EffectiveType) {
                            continue;
                        }
                    } else if(conditions.EffectiveType__c == 'null') {
                        if(EffectiveType) {
                            continue;
                        }
                    } else if (conditions.EffectiveType__c != EffectiveType) {
                        continue;
                    }
                }

                if ('EffectiveDate__c' in conditions) {
                    if (conditions.EffectiveDate__c == 'not null') {
                        if(!EffectiveDate) {
                            continue;
                        }
                    } else if(conditions.EffectiveDate__c == 'null') {
                        if(EffectiveDate) {
                            continue;
                        }
                    } else if (conditions.EffectiveDate__c != EffectiveDate) {
                        continue;
                    }
                }
    ...
    getConfigState: function(result) {
        ...
        var effectiveDateFormat = mItem.EffectiveDate__c ? new Date(mItem.EffectiveDate__c).toDateString() : '';
        ...
        //Schedule Message
        var confirmedSchedule = 'Action has been scheduled at '+effectiveDateFormat;
        var invalidEffectiveDate = 'The effective type is "schedule", but the effective date is invalid.';
        var notConfirmedSchedule = 'The effective type is "schedule" and the setting time on '+effectiveDateFormat+', Do you confirm to create the schedule?'
        ...
        var configState = [
            ...
            //Edit
            //-> START handle effective type = schedule
            {
                msg: confirmedSchedule,
                conditions: [
                    {
                        RecordTypeContains: ['CustomerEdit','SupplierEdit'],
                        isEffectiveDateFuture: true,
                        EffectiveType__c: 'Schedule',
                        EffectiveDate__c: 'not null',
                        SAPStatus__c: 'Effective Date Confirmed',
                    }
                ]
            },
            {
                msg: notConfirmedSchedule, //sap status != Effective Date Confirmed
                btnList: [
                    {
                        label: 'Submit',
                        action: 'submitWithGeneral'
            ...
            //-> END handle effective type = schedule
