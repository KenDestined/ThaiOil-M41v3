({
    doInit: function (component, event, helper) {
        ...
        Promise.all([p3, p4, p5, p6]).then($A.getCallback(function ([committeeInfo, groupedCommittee, trcrList, trcrHeadList]) {
            ...
            if (!committeeInfo.FinCrCond__c) {
                ...
                helper.setRequirePaymentCondition(committeeInfo.FinCrCond__c);
                helper.setRequireCreditLimit(committeeInfo.FinCrCond__c);
            }
    ...
    handleSave: function (component, event, helper) {
        ...
        if(isRequiredError) {
            helper.showToast('Final Condition: Please select ‘Yes’ at least one.', 'error');
            return;
        }
    ...
    handleChangeFinalCreditCondition: function (component, event, helper) {
        ...
        if (bu === 'TX') {
            const creditCondition = $A.util.isArray(component.find("TXFinCrCond"))
                ? component.find("TXFinCrCond")[0].get("v.value")
                : component.find("TXFinCrCond").get("v.value");
            helper.setFinalCreditConditionVisibility(creditCondition);
            helper.setRequirePaymentCondition(creditCondition);
            helper.setRequireCreditLimit(creditCondition);
    ...
    handleChangeTXCashOnDelivery: function (component, event, helper) {
        ...
        setTimeout($A.getCallback(function () {
            if (CashOnDelivery.get("v.value") !== "No") {
                HavingCollateral.reportValidity();
                BuyTradeEndorsement.reportValidity();
                BuyTradeDCLCondition.reportValidity();
                HavingOpenedCredit.reportValidity();
            }
        }), 250);
    },
    ...
    handleChangeTXCreditInsurance: function (component, event, helper) {
        ...
        if(value == 'Yes') {
            ...
            if(fieldName == 'FinBuyTradeEndorsement__c') {
                // component.find('BuyTradeEndorsement').set('v.value',);
                component.find('TXFinBuyTradeDCLCondition').set('v.value','No');
            } else if(fieldName == 'FinBuyTradeDCLCondition__c') {
                component.find('TXFinBuyTradeEndorsement').set('v.value','No');
            }
        }
    ...
    handleChangeCurrency: function (component, event, helper) {
        try {
            if (helper.getBU() === "TX") {
                component.find("TXFinTotalSecuredCurrency").set("v.value", component.find("TXFinCrLimitCur").get("v.value"))
            } else {
    ...
    handleLoadForm : function (component, event, helper) {
        ...
        if(creditLimitCmp && creditLimitCmp.get('v.value') != 'Unlimited' && creditLimitCmp.get('v.value') && !creditLimitCmp.get('v.value').includes(',')) {
        ...
        console.log('Set handleLoadForm',committeeInfo);
        if(committeeInfo.FinCrLimit__c && !component.get('v.requestFormObj.FinCrLimit__c')) {
            component.set('v.requestFormObj.FinCrLimit__c',committeeInfo.FinCrLimit__c);
        }

        if(committeeInfo.FinCrLimitCur__c && !component.get('v.requestFormObj.FinCrLimitCur__c')) {
            component.set('v.requestFormObj.FinCrLimitCur__c',committeeInfo.FinCrLimitCur__c);
        }
    },
})