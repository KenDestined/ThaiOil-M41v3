({
    ...
    handleUploadFinished: function (component, event, helper) {
        ...
        helper.getAttachmentsByIds(component, event, helper, files)
            ...
            .then(
                $A.getCallback((returnValue) => {
                    if (returnValue && $A.util.isArray(returnValue.filesInvalid)) {
                        const fileInvalidIds = returnValue.filesInvalid.map(file => file.Id);
                        helper.deleteAttachments(component, event, helper, fileInvalidIds);
                    }
                    if (returnValue && $A.util.isArray(returnValue.filesValid)) {
                        const newAttachments = component.get("v.newAttachments");
                        returnValue.filesValid.forEach((file) => {
                            newAttachments.push({
                                Id: file.Id,
    ...
    handlePreviewAttachment: function (component, event, helper) {
        helper.previewAttachment(component, event.currentTarget.getAttribute("data-value"));
    },
