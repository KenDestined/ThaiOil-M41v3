({
    ...
    checkAttachmentRulesRejectAll: function (component, event, helper, files) {
        return new Promise($A.getCallback(function (resolve, reject) {
            const errorList = [];
    ...
    checkAttachmentRules: function (component, event, helper, files) {
        return new Promise($A.getCallback(function (resolve, reject) {
            ...
            if ($A.util.isArray(files)) {
                ...
                const filesNotOverLimit = files.filter($A.getCallback(function (file) {
                    return !helper.isNonImageSizeOverLimit(component, helper, file);
                }));
                if (filesOverLimit.length > 0) {
                    errorList.push("The file size exceeds the limit allowed (at 5 MB per file) and cannot be attached");
                ...
                const totalFilesSizeOverLimit = helper.isTotalSizeOverLimit(component, helper, filesNotOverLimit);
                const lastIndex = filesNotOverLimit.length;
                let filesValid;
                let filesInvalid;
                if (totalFilesSizeOverLimit) {
                    errorList.push("The total file size exceeds the limit allowed (at 25 MB) and cannot be attached");
                    for (let i = 0; i < lastIndex; i++) {
                        filesValid = filesNotOverLimit.slice(0, lastIndex - i);
                        if (!helper.isTotalSizeOverLimit(component, helper, filesValid)) {
                            filesInvalid = filesNotOverLimit.slice(lastIndex - i);
                            break;
                        }
                    }
                } else {
                    filesValid = filesNotOverLimit;
                    filesInvalid = [];
                }

                if (filesOverLimit.length > 0 || filesNumberOverLimit || totalFilesSizeOverLimit) {
                    component.set("v.errorList", errorList);
                    filesInvalid.concat(filesOverLimit);
                }
    ...
    previewAttachment: function (component, contentDocumentId) {
