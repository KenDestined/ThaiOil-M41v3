({
    ...
    calTotalAmont: function(component, event, helper) {
        ...
        if(RecTypeName == 'Customer')
        {
            ...
            var totalCreditAmount = Number(amount2) + Number(amount3) + Number(amount4) + Number(amount5);
            ...
            if(buInfo.BusinessUnit__c == 'TX')
            {
                // var creditCurrency = component.find('CreditLimitCurrency')
                // if (creditCurrency) {
                //     var totalCurrency = component.find("Total_Secured_Currency__c").get("v.value");
                //     creditCurrency.set('v.value',totalCurrency)
                // }
                var creditLimit = component.find('CreditLimit')
    ...
    handleSubmit : function(component, event, helper) {
        ...
        // console.log('Submit Debug credit limit ',component.find('ApprovalTrader_CreditLimit__c').get('v.value'),component.find('TDCreditLimitHidden').get('v.value'))
        // if(component.find('ApprovalTrader_CreditLimit__c')) {
        //     component.find("TDCreditLimitHidden").set("v.value",component.find('ApprovalTrader_CreditLimit__c').get('v.value'));
        // }
        // console.log('Debug test',component.find('ApprovalTrader_CreditLimit__c').get('v.value'),component.find("TDCreditLimitHidden").get("v.value"))
        if(ButtonLabel1 != undefined && ButtonLabel1 != '')
        {
            ...
            try
            {
                ...
                if(component.find('ApprovalTrader_CreditLimit__c')) {
                    fields.ApprovalTrader_CreditLimit__c = component.find('ApprovalTrader_CreditLimit__c').get('v.value');
                }
                
                console.log('---RecTypeName--'+RecTypeName,fields.CreditOwner__c);
                ...
                if(checkAmountPass)
                {
                    ...
                    if(ButtonLabel1 == 'Save')
                    {
                        ...
                        fields.ApproverStep__c = ApproverStep;
                    }
                    ...
                    console.log('Debug formdata to be updated',JSON.parse(JSON.stringify(fields)))
                    debugger;
        ...
        if(ButtonLabel1 != undefined && ButtonLabel1 != '')
        {
            window.setTimeout(
                $A.getCallback(function() {
                    ...
                    console.log('handleSuccess after 3 second reload')
                    debugger;
                    window.location.reload();
                    component.set("v.ButtonLabel",'');
                }), 3000
    ...
    handleSaveFunction: function(component, event, helper) {
        try
        {
            ...
            var isRequiredError = component.get('v.IsShowRequiredMsg');
            if(isRequiredError) {
                var toastEvent = $A.get("e.force:showToast");
                toastEvent.setParams({
                    "title": "Error!",
                    "message": 'Credit Information Summary: Please select ‘Yes’ at least one.',
                    "type" : "error"
                });
                toastEvent.fire();
                return;
    ...
    handleRenderPicklist: function(component, event, helper) 
    {
        ...
        else
        {
            ...
            console.log('Debug BuyTradeEndorsement/BuyTradeDCLCondition',BuyTradeEndorsement,BuyTradeDCLCondition)
            if(BuyTradeEndorsement == 'Yes')
            {
                component.set('v.Yes3IsDisable',false);
                component.set('v.Ispass_Excelsummary',true);
    ...
    */
    ...
    handleCreditCondition: function(component, event, helper) {
        ...
        if(component.get('v.isTDCreditLoadDefault')) { //change condition when load default from TRCR to Trader => skip
            component.set('v.isTDCreditLoadDefault',false);
            return;
        }
        ...
        try{
            ...
            if(buInfo.BusinessUnit__c == 'TOP' || buInfo.BusinessUnit__c == 'LABIX')
            {
                ...
                else if(ApproverStep == 'Trader')
                {
                    ...
                    //Reset Value
                    // console.log('Debug approval credit limit',component.find('ApprovalTrader_CreditLimit__c'))
                    component.find("ApprovalTrader_CreditLimit__c").set("v.value",'');
                    component.find("TDCreditLimitHidden").set("v.value",'');
                    ...
                    console.log('******Onchange handleCreditCondition reset paymentterm')
                ...
                else if(CreditConditionValue == 'Open Account With Collateral') 
                {
                    if(ApproverStep == 'TRCR')
                    {
                    ...
                    else if(CreditConditionValue == 'Cash in Advance' || CreditConditionValue == 'Others' || CreditConditionValue == 'L/C' || CreditConditionValue == 'Domestic L/C' )
                    {
                        if(ApproverStep == 'TRCR')
                        {
                            component.set("v.IsTradeCreditDisable", false);
    ...
    handleTDNoWaiveRequest : function(component,event, helper) {
        ...
        if(TDNoWaiveRequestVal == 'No')
        {
            ...
            var TDPaymentTermCmp = component.find('ApprovalTraderPaymentTerm')
            console.log('Debug TDPaymentTermCmp',TDPaymentTermCmp,TDPaymentTermCmp.get('v.value'))
            if(TDPaymentTermCmp && !TDPaymentTermCmp.get('v.value')) {
                component.set('v.isTDCreditLoadDefault',true);
            }
            helper.defaultTraderSection(component,event,helper); //copy value from TRCR
            helper.handleTraderCreditField(component,event,helper); //Handle disable field
        }
        else
        {
            component.set('v.isTDCreditLoadDefault',false);
    ...
    handleOnloadForm : function(component,event, helper) {
        var listDefaultNA = ['InternalCreditRating','InternalCreditRatingTOP','ApprovalTrader_CreditRating'];
        console.log('Handle onload form')
        listDefaultNA.forEach(fieldId => {
            console.log('Debug foreach',fieldId)
            var cmp = component.find(fieldId);
            console.log('Debug defaultfield',fieldId,cmp.get('v.value'),cmp)
            if(cmp && !cmp.get('v.value')) {
                cmp.set('v.value','N/A')
            }
        });

        var creditLimitCmp = component.find('CreditLimitFormula');
            console.log('Debug 1',creditLimitCmp)
        if(creditLimitCmp && creditLimitCmp.get('v.value') != 'Unlimited' && creditLimitCmp.get('v.value')) {
            if(!creditLimitCmp.get('v.value').includes(','))
            {
                console.log('Debug 1.5',creditLimitCmp)
    ...
    var tdCreditLimitCmp = component.find('ApprovalTrader_CreditLimitType__c')
    	console.log('Debug 2',tdCreditLimitCmp);
        if(tdCreditLimitCmp && tdCreditLimitCmp.get('v.value') != 'Unlimited' && tdCreditLimitCmp.get('v.value') ) {
    		if(!tdCreditLimitCmp.get('v.value').includes(','))
            {
                console.log('Debug credit limit type',tdCreditLimitCmp.get('v.value'))
                var formattedValue = Number(tdCreditLimitCmp.get('v.value')).toLocaleString('en',{minimumFractionDigits: 2,maximumFractionDigits:2});
                tdCreditLimitCmp.set('v.value',formattedValue);
			}
            
        }

        var buInfo = component.get("v.BUInfo"); 
        console.log('Debug handleoload ---buInfo.BusinessUnit__c---'+buInfo.BusinessUnit__c);
        if(buInfo.BusinessUnit__c == 'TX')
        {
            var creditCurrency = component.find('CreditLimitCurrency')
            if (creditCurrency) {
            ...
            // console.log('TotalCreditAmount : ' , TotalCreditAmount, component.find("Amount7").get("v.value"));
            if (creditLimit) {
                var totalAmount = component.get('v.OriginalTotalCreditAmount')
                creditLimit.set('v.value',totalAmount)
            }

            var creditLimitFormula =  component.find('CreditLimitFormula');
            if (creditLimitFormula) {
                var totalAmount = component.get('v.OriginalTotalCreditAmount')
                creditLimitFormula.set('v.value',totalAmount)
            }
        }
...
console.log('Debug 3');
        if(component.get('v.isInitial')) {
            component.set('v.showLoading',false);
            component.set('v.isInitial',false);
        }
    },

    handleChangeTotalCurrency: function(component,event, helper) {
        console.log('--- handleCreditLimitCurrency ---');
        var buInfo = component.get("v.BUInfo"); 
        console.log('---buInfo.BusinessUnit__c---'+buInfo.BusinessUnit__c);
        if(buInfo.BusinessUnit__c == 'TX')
