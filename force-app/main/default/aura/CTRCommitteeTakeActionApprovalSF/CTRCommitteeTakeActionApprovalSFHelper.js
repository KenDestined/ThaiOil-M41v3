    ...
    getCTRCommitteeApprovalId: function (component) {
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS") {
                ...
                //     this.toastEvent('Error','Do not have permission ttt','error');
                // } else {
                //     component.set("v.CTRCommitteeApprovalId", response.getReturnValue());
                // }

                if(returnValue){
                    if(returnValue === 'ApprovedAgain'){
                        this.closeModal(component);
                        this.toastEvent('Error','You have already taken action on the specified process','error');
                    }
                    else{
                        component.set("v.CTRCommitteeApprovalId", response.getReturnValue());
                    }
                } else {
                    console.log('Debug do not have permission error')
                    this.closeModal(component);
